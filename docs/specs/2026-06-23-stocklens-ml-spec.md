...
| **D2** ⚠ pending | `predictions` пишет **API при инференсе** (идемпотентный upsert). **Меняет инвариант §2** — ждёт согласования владельца + обновления главной спеки §4 и `CLAUDE.md` инв. #2 отдельным коммитом (см. §8.4). | §8.5: «версия пишется в `predictions`»; инференс в API (§9.6). Единственный write-путь сохраняется. |
...
Порядок строгий: **сплит-коррекция → total-return c дивидендами → исключение weekend →
обрезка по `TRAIN_START`**.
...
2. **Total-return с дивидендами.** Для доходностей фич доходность в день, следующий за
   дивидендным гэпом, корректируется на величину дивиденда. **Семантика `ex_date`:** ingestor
   пишет `ex_date = MOEX registryclosedate` — это **T+1** (на один торговый день позже истинной
   ex-date, см.
   ...
   = дивиденд на акцию (приведённый к валюте цены — рубли; не-рублёвые дивиденды
...
3. **Исключение weekend-сессий.** Строки `is_weekend_session = true` (сессии выходного дня
   MOEX, торгуются с 03.2025) **исключаются** из обучения и расчёта фич (§8.1, риск-таблица
   главной спеки). Календарь фич строится по обычным торговым дням.
...
> **⚠ Ждёт согласования владельца (инвариант §2).** Инвариант §2 главной спеки и инвариант
> #2 в `CLAUDE.md` перечисляют write-владение API как **только** `portfolio_positions` и
> `bot_subscriptions`. D2 расширяет его на `predictions`. Субординированный документ **не
> может** менять родительский инвариант самозаявлением — требуется явное согласование
> владельца и обновление главной спеки §4 + `CLAUDE.md` инвариант #2 **отдельным коммитом**.
> До этого статус write-владения `predictions` — **pending**, не «решено». Спор решается до
> реализации serving-слоя (§8). Альтернатива, если владелец против расширения: писать
> `predictions` отдельным батч-джобом, а не API (тогда API только читает) — но это
> противоречит §8.5/§9.6 (инференс в API), поэтому рекомендация — расширить инвариант.
